...
# echo "peer 0 joining '$CHANNEL_NAME'"
...
echo "peer 1 joining '$CHANNEL_NAME'"
docker exec cli-peer1.org2 peer channel fetch 0 $CHANNEL_NAME.pb -o orderer.example.com:7050 --tls --cafile $ORDERER_TLS_CA -c $CHANNEL_NAME

docker exec cli-peer1.org2 peer channel join -b /etc/hyperledger/channels/$CHANNEL_NAME.block
docker exec cli-peer1.org2 peer channel list
